...
class MapProcessor(Node):
    ...
    def marker_callback(self, msg: MarkerArray):
        ...
        # self.get_logger().info(f"Markers Distances: " + str(self.marker_distances))
    ...
    def get_marker_distances(self, marker_array):
        ...
        try:
            ...
            self.get_logger().warn(f'Could not transform base_link to world: {e}')
    ...
    def publish_contour_markers(self, contour_centers, map_info, red_color = 1.0, green_color = 0.0, blue_color = 0.0):
    ...
    def process_map(self, map_msg):
        ...
        # self.get_logger().info(f"Detected {len(contour_centers)} contour centers: {contour_centers}")
        # Pulbish markers with the corrected midpoints
        self.publish_contour_markers(contour_midpoints, map_msg.info, red_color=0.0, blue_color=0.0, green_color=1.0)
        # self.get_logger().info(f"Midpoint {len(contour_midpoints)} contour centers: {contour_midpoints}")
